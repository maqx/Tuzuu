  ...
  <div class="available-balance">
    ...
    <div class="right-content">
      ...
      <div class="detail">
        <div class="list-top">
          <el-col :span="21">
            <span><i class=" green-txt font-size-22 iconfont">&#xe6a6; </i> 总计（元） </span> <span class="num red-txt"> {{dataList.account}}</span>
          </el-col>
          <el-col v-if="dataList.total" :span="3"><el-button class="green_btn">全部导出</el-button></el-col>
          <el-col  v-if="!dataList.total"  :span="3"><el-button class="gray_btn">全部导出</el-button></el-col>
        </div>
        <div v-loading="loading" class="lists">
          ...
          <div class="list"  v-if="dataList.total">
            <ul class="item" :key="item.order_detail_id"  v-for="item in dataList.list">
              <li class="time">{{item.time}}</li>
              <li class="record-id" v-if="!item.order_detail_id">{{item.record_id}}</li>
              <router-link v-else :to="{ path:'/order/detail/'+item.order_detail_id}"  class="record-id font-size-14 green-txt">{{item.record_id}}</router-link>
            ...
            <pages :currentPage="currentPage" :total="dataList.total"  :handleCurrentChange="handleCurrentChange"></pages>
          </div>
          <div v-else>
            <div class="no-order">
              <div class="pic"></div>
  ...
  export default {
        ...
        leftTab:{
          ...
          list:[
            {title:'个人账户',id:1,to:'/account',active:'active',childLists:[
              ...
              {title:'可提现金额流水',id:'b',to:'/account/availableBalance',active:'childActive'},
    ...
    methods: {
      handleCurrentChange(val) {
        ...
        this.loading=true
        this.param.page = val
        this.getAvailableBalanceApi(this.param)
      },
      handleSizeChange(){

      },
      getAvailableBalanceApi(param){
        api.getAvailableBalanceApi(param)
          .then(res=>{
            if(res.status){
